...
app.get("/popularinFan", async (req, res) => {
	let products = await Product.find({category:"Fans"});
  let popular_in_Fans = products.splice(0,  4);
  console.log("Popular In Fans");
  res.send(popular_in_Fans );
});
...
app.post("/neworder",fetchuser, async (req, res) => {
  let userData = await Users.findOne({_id:req.user.id});
  let orders = await Order.find({});
  let id;
  if (orders.length>0) {
